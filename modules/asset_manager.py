...
def get_hybrid_video(keyword: str, vibe: str, orientation: str):
    ...
    # 1. Pexels (Supporta filtro nativo)
    if pex_key:
        video = _search_pexels(clean_keyword, vibe, orientation, pex_key)
        if video: return video | {"source": "Pexels"}
    
    # 2. Pixabay (Simuliamo il filtro con la query)
    if pix_key:
        video = _search_pixabay(clean_keyword, vibe, orientation, pix_key)
...
def _search_pexels(keyword, vibe, orientation, key):
    ...
    # Pexels accetta 'portrait' o 'landscape'
...
def _search_pixabay(keyword, vibe, orientation, key):
    # Pixabay non ha orientation nei video, lo aggiungiamo alla query
    # Se landscape, non aggiungiamo nulla (è il default), se portrait aggiungiamo "vertical"
    query_suffix = " vertical" if orientation == "portrait" else ""
    query = f"{keyword}{query_suffix}"
...
# (Mantieni get_background_music invariato)
